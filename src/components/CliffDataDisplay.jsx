...
function CliffDataDisplay() {
  ...
  const [bodyImages, setBodyImages] = useState([]);
    ...
    useEffect(() => {
    fetch(`${API_BASE}/cliffTeam`)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      .then(json => {
        console.log('=== DEBUG CLIFF DATA ===');
        console.log('Headers:', json.headers);
        console.log('Headers length:', json.headers?.length);
        console.log('Header images:', json.header_images);
        console.log('Header images length:', json.header_images?.length);
        console.log('Body images length:', json.body_images?.length);
        console.log('Body images[0]:', json.body_images?.[0]);
        console.log('Body images[0] length:', json.body_images?.[0]?.length);
        ...
        console.log('Header images set:', headerImgs.slice(0, headersLength));
        console.log('Body images set:', bodyImgs);
      })
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className={styles.tableWrapper}>
        <div className={styles.description}>
          <h3>Cliff is one of the three Team GO Rocket Leaders. He always leads with the first Pokémon on this page.</h3>
        </div>
        <div className={styles.subtitle}>
          <h2>Cliff's Pokémon</h2>
          <h3>These are the Pokémon Cliff uses in his current lineup with reccommendations to beat them. <br />If you don't have Mega or Shadow versions of these Pokémon, regular versions can still be effective.</h3>
          <h3>Note that Cliff's Pokémon may change in the future, so this data may not always be accurate.</h3>
